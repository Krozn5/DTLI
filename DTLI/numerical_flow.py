...
def plot_dist(keys: np.ndarray, path, fig_type, normalize=False):
  ...
  if fig_type == 'pdf':
    ...
    plt.plot(x.tolist(), y,color='crimson')
    plt.scatter(x.tolist(), y, s=7,c='darkgray')
  plt.title(fig_type)
...
if __name__ == '__main__':
  ...
  '''数据名称'''
  parser.add_argument('--data_name', type=str, default='amz-ts-200M-100R-zipf')
  parser.add_argument('--seed', type=int, default=1000000013)
  ...
  num_train_keys = np.max((np.clip(int(load_keys.shape[0] * args.train_ratio), 
                          1, load_keys.shape[0]), 10000))#todo:
  train_keys_list = []
